defmodule Ambry.Images do
  ...
  and `Ambry.Inbox` cannot call into `AmbryWeb`.
  `AmbryWeb.Admin.UploadHelpers` delegates here.

  Everything written here is Ambry's own, under the uploads path, never a file
  inside a library root, which is what makes these safe to delete with a
  ...
  # A `content-type` allowlist is not enough: provider CDNs routinely label
  # every asset `application/octet-stream`, so a PNG picked from the person
  # picker passes `valid_url?` on its extension and then fails the download,
  # silently, since a failed
  ...
  **It does not assume those bytes are a JPEG.** Naming the output
  `image/jpeg` and stream-copying whatever was in there into it produces a
  file whose extension lies about its content, and a browser says so ("the
  jpeg contains errors"). Thumbnails would still look fine, because libvips
  reads by content and ignores the name; only the original, which is what the
  form renders, would be broken.

  So the bytes decide the format and the extension, and anything a browser
  cannot show is re-encoded on the way in. That is `read_embedded/1`'s job
  exactly, and this is that plus a file: one extraction path, one policy
  ...
  bytes say what it is and the filename cannot. What they say is not
  necessarily what the *container* says: a file can declare a PNG stream and
  carry a big-endian TIFF (`4D 4D 00 2A`), which ffprobe itself complains
  about. Such a picture extracts fine, fails the sniff, and 404s into a broken
  image sitting next to a chip offering it, for the one decision where seeing
  the art is the entire point.
