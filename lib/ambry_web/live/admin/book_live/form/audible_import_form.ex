defmodule AmbryWeb.Admin.BookLive.Form.AudibleImportForm do
  ...
  def handle_async(:select_book, {:ok, results}, socket) do
    %{selected_book: selected_book, matching_authors: matching_authors, matching_series: matching_series} = results
  ...
  def handle_async(:select_book, {:exit, {exception, _stacktrace}}, socket) do
    {:noreply, assign(socket, selected_book: AsyncResult.failed(socket.assigns.selected_book, exception.message))}
  end
    ...
    params =
      Enum.reduce(import_params, %{}, fn
        ...
        {"use_authors", "true"}, acc ->
          Map.put(acc, "book_authors", build_authors_params(book.authors, socket.assigns.matching_authors.result))

        {"use_series", "true"}, acc ->
          Map.put(acc, "series_books", build_series_params(book.series, socket.assigns.matching_series.result))
