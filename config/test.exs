...
# There is no search index listener in test: the SQL sandbox holds every write
# in a transaction that never commits, so the `NOTIFY` the enqueue trigger
# raises is never delivered. Reads drain the queue inline instead, which puts
# a test in the same position as a caller a millisecond after a write.
# We don't run a server during test. If one is required,
# you can enable the server option below.
config :ambry, Ambry.Search, settle_inline: true
